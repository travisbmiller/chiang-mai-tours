...
import { Users, ChevronDown, ChevronRight, ChevronLeft, X, Menu, Phone } from "lucide-react";
...
function BookingContent() {
  ...
  // Local state for form inputs (replaces props)
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(
    initialDate ? new Date(initialDate) : undefined
  );
  const [adults, setAdults] = useState(initialAdults);
  const [childrenCount, setChildren] = useState(initialChildren);
  ...
  // Our tours infinite scroll state
  ...
  // Infinite scroll setup - position at middle set on mount
  useEffect(() => {
  ...
  }, [showConfirmation, cardWidth, totalCards]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      const childrenText = childrenCount > 0 ? (childrenCount === 1 ? ", 1 Child" : `, ${childrenCount} Children`) : "";
      const peopleText = adultsText + childrenText;

      const selectedTourNames = formData.selectedOtherTours.map(id => {
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="space-y-8">
              ...
              <div>
                ...
                <ul className="space-y-3">
                  <li className="flex items-center space-x-2 text-[#000000]">
      ...
      {showConfirmation ? (
        ...
        <>
          <div className="max-w-2xl mx-auto px-8 pt-8 pb-16">
            ...
            <div className="w-48 h-48 mx-auto mb-8 relative">
              <div className="absolute inset-0 bg-gradient-to-b from-white/80 to-white/40 rounded-full" style={{ padding: '8px' }}>
                <div className="w-full h-full rounded-full bg-white shadow-lg" style={{ padding: '4px' }}>
                  <img
            ...
            <h2 className={`text-4xl md:text-5xl font-bold text-[#000000] mb-6 ${dmSerifDisplay.className}`}>
              Request sent.
            </h2>

            {/* Message text - no heading */}
            {formData.usesWhatsApp === true && (
              <p className="text-lg text-[#000000] mb-4">
                For a faster response, please contact me via WhatsApp.
              </p>
            )}

            {/* Messaging Buttons Container */}
            <div className="w-full max-w-md mx-auto space-y-3 mb-12">
              {/* WhatsApp Button */}
              {formData.usesWhatsApp === true && (
                <Button
                  onClick={() => {
                  ...
                  message += `\n*Tour Cost:*\n`;
                  message += `- Guests (${guests} x ฿${basePrice.toLocaleString()}): ฿${guestsTotal.toLocaleString()}\n`;
                  if (includeMorningAlms) {
                    message += `- Morning Monk Alms (Tour Add-on): ฿${morningAlmsAddon.toLocaleString()}\n`;
                    message += `- Monk Alms Offering (${guests} x ฿100): ฿${almsOffering.toLocaleString()}\n`;
                  }
                  message += `- *Total: ฿${total.toLocaleString()} Baht*\n`;

                  message += `\n*Preferences:*\n`;
                  message += `- Licensed Guide: ${formData.needsGuide === null ? 'Not specified' : formData.needsGuide ? 'Yes' : 'No'}\n`;
                  message += `- WhatsApp: ${formData.usesWhatsApp === null ? 'Not specified' : formData.usesWhatsApp ? 'Yes' : 'No'}\n`;

                  if (formData.interestedInOtherTours === true && selectedTourNames.length > 0) {
                ...
                className="w-full bg-[#25D366] hover:bg-[#20BD5A] text-white py-7 text-lg font-semibold flex items-center justify-center gap-3 transition-all duration-200"
            ...
            <div className="border border-gray-200 rounded-3xl p-8 mb-12 bg-white">
              <h3 className="text-base font-bold text-[#000000] mb-2">Thank you so much for your booking request.</h3>
              <p className="text-sm text-[#000000] leading-relaxed">
                It means a lot that you're interested in discovering Chiang Mai with me. I'll get back to you as soon as I can, and I can't wait to help you create a memorable journey through the mountains, temples, and hidden corners of this beautiful region.
              </p>
            </div>
            </div>
          </div>

          {/* Our tours section - Full Width */}
                  ...
                  <div className="flex space-x-6 py-6 px-4" style={{width: 'max-content'}}>
                    ...
                    {tourCards.map((tour, index) => (
                        <a
                          key={`end-${index}`}
                          href="/"
                          className="group relative rounded-xl overflow-hidden shadow-lg flex-shrink-0 cursor-pointer transition-all duration-300 hover:scale-105 hover:-translate-y-1 block"
                          style={{
                            width: '300px',
                            height: '200px',
        ...
        /* Booking Form */
        <div className="mx-auto md:px-8 pt-3 md:pt-8 pb-24 overflow-x-hidden" style={{ maxWidth: '1120px' }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Left Column - Form */}
            <div className="order-2 lg:order-1 px-4 md:px-0 pb-12">
            ...
            <form id="booking-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-6 lg:hidden">
                <Button
                  ...
                  onClick={() => router.back()}
                  variant="ghost"
                  className="flex-1 py-6 text-[#000000] rounded-full hover:bg-gray-100"
                >
                  Cancel request
                </Button>
                <Button
                  ...
                  className="flex-1 py-6 bg-[#000000] hover:bg-[#1a1a1a] text-white rounded-full disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <div className="order-1 lg:order-2 -mx-2 md:mx-0">
            <div className="bg-white rounded-none md:rounded-2xl border-b-2 md:border-2 border-gray-200 overflow-hidden lg:sticky lg:top-6 md:shadow-[0_0_40px_rgba(0,0,0,0.15)]">
              <div style={{ paddingLeft: '25px', paddingTop: '25px', paddingRight: '25px', paddingBottom: '0' }}>
                {/* Tour Title with Image */}
                <div className="flex items-start justify-between gap-6 mb-6">
                  <div className="flex-1">
                    <div className={`text-lg text-[#000000] ${dmSans.className}`}>{(() => {
                      const lastWordIndex = tourName.lastIndexOf(' ');
                      const lastTwoWordsIndex = tourName.lastIndexOf(' ', lastWordIndex - 1);
                      return tourName.substring(0, lastTwoWordsIndex) || "Discover the Magic of";
                    })()}</div>
                    <div className={`font-bold text-[#000000] leading-tight ${dmSerifDisplay.className}`} style={{ fontSize: '2.75rem' }}>
                      {(() => {
                        const lastWordIndex = tourName.lastIndexOf(' ');
                        const lastTwoWordsIndex = tourName.lastIndexOf(' ', lastWordIndex - 1);
                        return tourName.substring(lastTwoWordsIndex + 1) || "Doi Suthep";
                      })()}
                    </div>
                  </div>
                        ...
                        <div className="absolute z-50 w-full mt-2 p-4 bg-white border border-gray-200 rounded-xl shadow-lg space-y-4">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-0.5">
                              ...
                              <button
                                type="button"
                                onClick={() => setChildren(Math.min(10, childrenCount + 1))}
            ...
            {/* Buttons - Desktop Only (Below Card) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              ...
              <ul className="space-y-2 text-sm text-[#000000]">
                <li><a href="/" className="hover:text-rose-600">Doi Suthep</a></li>
                <li><a href="#" className="hover:text-rose-600">Doi Inthanon</a></li>
                <li><a href="#" className="hover:text-rose-600">Best of Pai</a></li>
                <li><a href="#" className="hover:text-rose-600">Chiang Rai & Golden Triangle</a></li>
                <li><a href="#" className="hover:text-rose-600">Sukhothai</a></li>
              </ul>
            </div>
